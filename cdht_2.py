...
# peer_port = int(sys.argv[1]) + base_port_num
# successor_1 = int(sys.argv[2]) + base_port_num
# successor_2 = int(sys.argv[3]) + base_port_num

# peer_id = sys.argv[1]
# peer_port = base_port + int(peer_id)
#
    ...
    def run(self):
        ...
        while True:
            if not self._running:
                break
            data = [seq, peer_id]
            client.sendto(bytes(data), (host, peer_successor_1_port))
            client.sendto(bytes(data), (host, peer_successor_2_port))
            seq += 1
            time.sleep(5)
            for _ in range(2):
                try:
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                # time.sleep(5)
                if data[1] not in predecessor:
                    predecessor.append(data[1])
...
def TCPClient_request(host, port, data):
    ...
    tcpsocket.send(bytes(data))
    response = tcpsocket.recv(1024)
    tcpsocket.close()
    if response:
        return response

# def TCPClient_response(host, port):
#     tcpsocket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
#     tcpsocket.connect((host, port))
#
#     tcpsocket.close()
...
class TCPServer(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            data = list(data)
            if data[0] == 0: # a peer quit gracefully
                print('Peer {} will depart from the network.'.format(data[1]))
                if peer_successor_1 == data[1]:
                    peer_successor_1 = data[2]
                    peer_successor_2 = data[3]
                elif peer_successor_2 == data[1]:
                    peer_successor_2 = data[2]
                ...
                data = [peer_successor_1, peer_successor_2]
                connectionSocket.send(bytes(data))

            else:  # request file, data[0]=filename, data[1]=predecessor
                if data[0] % 256 <= peer_id and data[0] % 256 > data[1] \
                        or data[0] % 256 > data[1] and peer_id < data[1] \
                        or data[0] % 256 <= peer_id and data[1] > peer_id:
                    print('File {} is here.'.format(data[0]))
                else:
                    print('File {} is not stored here.'.format(data[0]))
                    TCPClient_request(host, peer_successor_1_port, [data[0], peer_id])
